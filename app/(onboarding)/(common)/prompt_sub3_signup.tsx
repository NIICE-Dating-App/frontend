...
function PromptRow({
  ...
  return (
    <Pressable 
      ...
      hitSlop={6} 
      disabled={disabled}
      style={({ pressed }) => [pressed && !disabled && { opacity: 0.96 }]}
    >
      <View
        style={[
          styles.row,
          { 
            ...
            backgroundColor: disabled ? "#F5F7FA" : CARD_BG,
...
export default function PromptSubSignup() {
  ...
  // Fetch already used prompts from previous slots
  useEffect(() => {
    const fetchUsedPrompts = async () => {
      try {
        ...
        if (data && data[PROMPT_ARRAY_COL]) {
          const existing = Array.isArray(data[PROMPT_ARRAY_COL]) ? data[PROMPT_ARRAY_COL] : [];
          // Get all prompts from other slots
          const used = existing
            .filter((x: any) => x?.slot !== slot && x?.question)
            .map((x: any) => x.question);
          setUsedPrompts(used);
        }
      } catch (e) {
        console.error("Error fetching used prompts:", e);
      }
    };
  ...
  const onPickPrompt = (p: string) => {
    if (usedPrompts.includes(p)) return; // Don't allow selecting used prompts
    
    LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
    setSelectedPrompt((prev) => (prev === p ? prev : p));
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.card}>
          {prompts.map((p, i) => {
            const isUsed = usedPrompts.includes(p);
