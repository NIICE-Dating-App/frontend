...
export default function ChatScreen() {
  ...
  const handleChatPress = (conversation: ChatConversation) => {
    if (conversation.type === "group_chat" && conversation.groupId) {
      // Route to group chat page
      router.push({
        pathname: "/(tabs_support)/chat_talk_group",
        params: { groupId: conversation.groupId },
      });
    } else if (conversation.matchRequestId) {
      router.push({
                ...
                return (
                  <View style={styles.announcementCard}>
                    ...
                    <Text style={styles.announcementMeta}>
                      {a.authorName || "Admin"} Ã‚Â· {formatTime(a.createdAt)}
                    </Text>
                  </View>
                );
              }
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={styles.header}>
        ...
        {activeTab === "groups" && getFilteredConversations().length > 0 ? (
          <TouchableOpacity 
            style={styles.headerButton} 
            activeOpacity={0.7}
            onPress={() => router.push("/(tabs_support)/create_group")}
          >
            <Ionicons 
              name="add" 
              size={26} 
      ...
      {loading ? (
        ...
        getFilteredConversations().length > 0 ? (
          ...
          <EmptyState
            ...
            onPress={() => router.push(activeTab === "chat" ? "/(tabs)/map" : "/(tabs_support)/create_group")}
          />
        )
      )}
...
const ChatRow: React.FC<ChatRowProps> = ({ conversation, onPress }) => {
  const { name, avatarUrl, lastMessage, updatedAt, unreadCount, hasFrame, type } = conversation;
        ...
        <View style={[styles.avatarInner, hasFrame && styles.avatarWithFrame]}>
          {avatarUrl ? (
            ...
            <View style={styles.avatarPlaceholder}>
              <Ionicons name={isGroup ? "people" : "person"} size={22} color={BLUE} />
            </View>
          )}
        </View>
      </View>
  ...
  <TouchableOpacity style={styles.chatRow} activeOpacity={0.6} onPress={onPress}>
    ...
    <View style={styles.chatContent}>
      ...
      <Text style={styles.chatMessage} numberOfLines={1}>
        {community.memberCount.toLocaleString()} members Ã‚Â· {community.category}
      </Text>
    </View>
    <Ionicons name="chevron-forward" size={20} color="rgba(10,14,26,0.25)" />
  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  shadowColor: BLUE,  // Ã¢â€ Â ADD THIS LINE
  shadowOffset: { width: 0, height: 4 },  // Ã¢â€ Â ADD THIS LINE
  shadowOpacity: 0.25,  // Ã¢â€ Â ADD THIS LINE
  shadowRadius: 8,  // Ã¢â€ Â ADD THIS LINE
  elevation: 6,  // Ã¢â€ Â ADD THIS LINE
},
  tabButtonText: {
    fontFamily: Fonts.bold,
    fontSize: scale(15),
